...
const heatmapBuilder = (dataset, containerEl) => {
  ...
  //Read the data
  d3.csv("https://raw.githubusercontent.com/dvcarrillo/ivis-group-selection/dev/src/datasets/ivis.csv", function (data0) {
    let data = [];
    // Main matrix
    for (let i = 0; i < data0.length; i++) {
      for (let j = 1; j < data0['columns'].length; j++) {
        data.push({
          group: data0[i]['User'],
          variable: data0['columns'][j],
          value: data0[i][data0['columns'][j]]
        });
      }
    }
    ...
    const mouseover = function (d) {
      ...
      svg.selectAll(`.${d.group}`).selectAll('rect').style('opacity', 0.55);
      svg.selectAll(`.${d.variable}`).selectAll('rect').style('opacity', 0.8);
      svg.selectAll(`.${d.variable}`).selectAll('text').style('opacity', 0.8);
    ...
    const mouseleave = function (d) {
      ...
      svg.selectAll(`.${d.variable}`).selectAll('text').style('opacity', 0);
    ...
    const cells = svg.selectAll()
      ...
      .enter()
      .append("g")
      .attr('class', (d) => `${d.variable} ${d.group}`)
      .on("click", click)
